...
- (void)jsCancel {
    ...
    NSString* jsCallback = [NSString stringWithFormat:@"plugins.datePicker._dateSelectionCanceled();"];
    [self.commandDelegate evalJs:jsCallback];
}

- (void)jsDateSelected {
    NSTimeInterval seconds = [self.datePicker.date timeIntervalSince1970];
    NSString* jsCallback = [NSString stringWithFormat:@"plugins.datePicker._dateSelected(\"%f\");", seconds];
    [self.commandDelegate evalJs:jsCallback];
}
